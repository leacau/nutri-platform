		...
		mutationFn: async () => {
			...
			if (type === 'meal_plan') {
				if (mealPlanMode === 'free_text')
					dataPayload = { mode: 'free_text', freeTextContent: mealPlanContent };
				else dataPayload = { mode: 'weekly', weeklyTemplate: weeklyPlan };
			}
			if (type === 'measurement') {
				dataPayload = {
			...
			if (isEditing) {
				return apiClient.updateClinicalRecord(
					recordToEdit!.id,
					{ date: new Date(date).toISOString(), data: dataPayload },
					activeClinicId!,
					idToken ?? undefined,
				);
			} else {
