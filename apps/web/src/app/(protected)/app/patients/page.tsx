...
export default function PatientsPage() {
  ...
  const mutation = useMutation({
    mutationFn: async (data: PatientForm) => {
      ...
      return apiClient.createPatient(activeClinicId, data, undefined);
    },
    onSuccess: () => {
  ...
  const patients = useMemo(() => {
    ...
    return list.filter((patient) => {
      ...
      const matchesNutri = selectedNutri === "all" || patient.assignedNutriId === selectedNutri;
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr,400px]">
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Badge variant="secondary">{patientsQuery.data?.length ?? 0} registros</Badge>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-xl bg-white p-3 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={selectedNutri} onChange={(e) => setSelectedNutri(e.target.value)} className="w-56">
        ...
        <Card>
          <CardContent className="divide-y p-0">
            {patients.map((patient) => (
              <div key={patient.id} className="flex items-center justify-between gap-4 p-4">
                <div>
                  <p className="font-semibold">{patient.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {patient.email || "sin email"} · {patient.phone || "sin teléfono"}
                  </p>
            ...
            {!patients.length ? <p className="p-4 text-sm text-muted-foreground">No hay pacientes para los filtros aplicados.</p> : null}
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <form className="space-y-3" onSubmit={handleSubmit((data) => mutation.mutateAsync(data))}>
            <div className="space-y-1">
              <Label>Nombre</Label>
              <Input placeholder="Nombre y apellido" {...register("name")} required />
            </div>
            <div className="space-y-1">
              <Label>Email</Label>
            ...
            {mutation.error ? <p className="text-sm text-destructive">No pudimos crear el paciente.</p> : null}
          </form>
        </CardContent>
      </Card>
    </div>
