...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      console.error(err);
      setError("No pudimos iniciar sesión. Revisá tus datos o probá con Google.");
    }
  };
    ...
    <div className="flex min-h-screen">
      ...
      <div className="mx-auto flex w-full max-w-xl flex-col justify-center px-8 py-16">
        <div className="mb-8 space-y-2 text-center">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1 text-xs font-semibold text-primary">Acceso</div>
          <h1 className="text-3xl font-semibold text-primary">{t("hero.welcome")}</h1>
          <p className="text-sm text-muted-foreground">Ingresá con tu cuenta para continuar.</p>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="email">{t("auth.email")}</Label>
            <Input id="email" placeholder="vos@amsa.core" type="email" required {...register("email")} />
          </div>
          <div className="space-y-2">
